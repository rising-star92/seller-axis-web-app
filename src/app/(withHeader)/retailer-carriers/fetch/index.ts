...
export const getRetailerCarrierService = async ({
  ...
  return await httpFetchClient.get(`retailer-carriers?search=${search}&page=${page}&page_size=10`);
};

export const getServicesService = async ({ search, page }: { search: string; page: number }) => {
  const httpFetchClient = new fetchClient();

  return await httpFetchClient.get(`services?search=${search}&page=${page}&page_size=10`);
};
