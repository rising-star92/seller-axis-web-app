...
type Props = {
  resBulkShip: never[];
  handleCloseBulkShip: () => void;
  isLoadingShipment: boolean;
};

export default function ResultBulkShip({
  resBulkShip,
  isLoadingShipment,
  handleCloseBulkShip
}: Props) {
  ...
  const renderBodyTable = resBulkShip?.map((row: any) => ({
    id: row?.id || '',
  ...
  return (
    <div className="dark:header_cus header_cus_light fixed bottom-0 right-0 z-[20] w-auto animate-slideInLeft border bg-paperLight shadow-lg ring-1 ring-black ring-opacity-5 dark:bg-darkGreen">
      <div className="flex items-center justify-between py-[8px]">
        <span className="pl-3 text-sm font-normal">{resBulkShip?.length} Rows Selected</span>
        <Button onClick={handleCloseBulkShip}>
          <CloseIcon />
        </Button>
      </div>
      <Table
